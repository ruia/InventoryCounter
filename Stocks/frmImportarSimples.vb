...
Public Class frmImportarSimples
    ...
    Private Sub btnAbrirFicheiro_Click(sender As Object, e As EventArgs) Handles btnAbrirFicheiro.Click
        ...
        If (Me.ofdAbrirFicheiro.ShowDialog = DialogResult.OK) Then
            ...
            produtos.RemoveAt(0)
            Dim fornecedor As String = Me.txtFicheiro.Text.Split("\").Last().Replace(".csv", "")
            For Each linha As String In produtos
                Me.txtProdutoAtual.Text = linha
                Dim tmp() As String = linha.Split(";")
                p.codigo = fornecedor & "-" & RemoveWhitespace(tmp(0).Trim)
                p.descricao = tmp(1).Trim
                p.preco = RemoveWhitespace(tmp(3).Trim.Replace(",", ".").Replace("€", ""))

                p.cb = RemoveWhitespace(tmp(2).Trim)
    ...
    Private Sub btnImportar_Click(sender As Object, e As EventArgs) Handles btnImportar.Click
        Using conexao As New SqlConnection With {.ConnectionString = Globais.connectionString}
            ...
            For i As Integer = 0 To Me.dgv.RowCount - 1
                Using cmd As New SqlCommand With {.CommandText = "SELECT CodBarras FROM Produtos WHERE CodBarras=@CodBarras", .Connection = conexao}
                    cmd.Parameters.AddWithValue("CodBarras", Me.dgv(3, i).Value.ToString)
                    Me.txtnAct.Text = "Registo atual nº: " & i + 1
                    Me.txtProdutoAtual.Text = Me.dgv(0, i).Value.ToString
                    Dim dataReader As SqlDataReader = cmd.ExecuteReader
                    If (dataReader.HasRows) Then
                        Using cmdUpdate As New SqlCommand With {.CommandText = "UPDATE Produtos SET Descricao=@Descricao, CodBarras=@CodBarras, PrecoCusto=@PrecoCusto WHERE CodBarras=@CodBarras", .Connection = conexao}
